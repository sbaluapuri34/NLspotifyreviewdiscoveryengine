            ...
            if packages_path.exists():
                try:
                    ...
                    for idx, pkg in enumerate(packages):
                        ...
                        cluster_name = pkg.get("cluster_name") or fallback_name
        ...
        if packages_path.exists():
            with open(packages_path, "r", encoding="utf-8") as f:
                ...
                for pkg in packages:
                    ...
                    sub_themes_map[cid] = pkg.get("sub_themes", [])
                    jtbd_map[cid] = pkg.get("jtbd", {})
                    workarounds_map[cid] = pkg.get("workarounds", [])
        ...
        if not answers_list:
            ...
            if json_path.exists():
                try:
                    ...
                    logger.info("Loaded research answers from fallback JSON.")
                except Exception as json_err:
        ...
        if not loaded_insights:
            ...
            if json_path.exists():
                try:
                    ...
                    logger.info(f"Loaded executive insights from fallback JSON: {json_path.name}")
                except Exception as json_err:
