...
class ResearchEngine:
    ...
    def synthesize_rq_answer(self, rq_id: str, relevant_packages: List[Dict[str, Any]], previous_answer: Optional[str] = None) -> Dict[str, Any]:
        ...
        for pkg in relevant_packages[:6]:  # Limit to top 6 most relevant clusters to save tokens to prevent rate limits
            ...
            # Include top 2 medoid quotes with review IDs
            medoids = pkg.get("medoids", [])
            if medoids:
                evidence_str += "  Key Quotes:\n"
                for m in medoids[:2]:
                    quote = m.get("translated_text") or m.get("raw_text") or ""
                    rid = m.get("id") or m.get("review_id") or "unknown"
                    evidence_str += f"    * [ID: {rid}] \"{quote[:150]}...\"\n"
            evidence_str += "\n"
...
4. Source Citations: For each key finding under "key_findings", you MUST explicitly list the supporting review IDs in an array of strings called `"citations"` (e.g., ["google_play_123", "reddit_456"]). Only cite review IDs that are explicitly provided in the Key Quotes under EVIDENCE PACKAGES.
