...
For each of the following feedback clusters, analyze its key themes and representative reviews (each provided with a `review_id` and a `text` field).

CRITICAL ANTI-HALLUCINATION & RAG RULES:
...
2. Workarounds: Only list workarounds that users explicitly mentioned in the provided reviews. If no workarounds are mentioned in the text, you MUST return an empty list `[]`. Do not invent generic workarounds.
...
4. Source Citations: For each sub-issue, you MUST populate a `"citations"` array of strings containing the exact `"review_id"` values of the reviews that support this sub-issue. Only cite IDs from the reviews list provided for that cluster.

For each cluster, analyze its reviews and:
1. Generate a short, descriptive, professional name/title (max 4-6 words) specific to the core issue.
...
5. Decompose the cluster into 2-3 specific "sub_issues", mapping each back to a parent sub-theme using an `associated_theme_id` and including its supporting review citations.
