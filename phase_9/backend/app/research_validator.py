...
CRITICAL ANTI-HALLUCINATION & RAG GROUNDING RULES:
...
3. Unsupported Claims: You must actively populate the `unsupported_claims_or_overgeneralisations` array with any claims in the Research Question answers that go beyond the actual cluster evidence provided or lack supporting review citations, enforcing database sanity.
4. Citation Audit: Audit the provided Research Question answers. Ensure key findings contain a populated `"citations"` list referring to valid review IDs from the inputs. Identify any that lack supporting review references.
