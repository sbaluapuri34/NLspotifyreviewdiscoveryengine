...
## 20. Strict RAG Grounding & Source-Citation Engine

To achieve complete academic auditability and prevent LLM hallucinations, the platform features a strict Retrieval-Augmented Generation (RAG) grounding architecture that ties qualitative insights directly to database review records:

### A. Review-ID Ingestion Mapping
*   **Structured Review Inputs**: During Step 4 (Batch Naming) and Step 5 (Research Synthesis), reviews are formatted not as raw strings but as structured JSON entities containing both `review_id` and the clean `text` payload.
*   **Prompt-Level ID Context**: Key quotes in the synthesized evidence packages are prefixed with their explicit database record identifiers (e.g., `[ID: google_play_1029]`), grounding the LLM's context window.

### B. Anti-Hallucination & Citation Schemas
*   **Citations Output**: The LLM prompts are enforced with JSON response schemas requiring a `"citations"` list of strings for all key findings:
    *   *Namer schema*: `"citations": ["string"]` under each sub-issue.
    *   *Research schema*: `"citations": ["string"]` under each key finding.
*   **Strict Negative Constraints**: System instructions forbid the generation of generic opportunities or workarounds. If there is no explicit citation in the inputs, the fields are required to default to empty arrays or write 'Not indicated in source reviews'.

### C. Validation Audit Gate (Level 3)
*   **Citation Audit**: The `ResearchValidator` parses the research answers and audits the citations. Any claims or findings lacking valid review ID citations are flagged, cataloged in the `unsupported_claims_or_overgeneralisations` validation log, and corrected before being committed to the SQLite database.
