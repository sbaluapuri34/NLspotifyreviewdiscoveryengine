    ...
    try {
        ...
        if (contextText && data.metadata) {
            ...
            if (meta.view_type === 'session') {
                htmlContent = `<strong>Temporary Session Analysis:</strong> ${meta.from_date} to ${meta.to_date} (${meta.total_reviews} reviews)`;
...
function selectCluster(cluster) {
    ...
    // Render themes
    const themesContainer = document.getElementById('detail-themes');
    themesContainer.innerHTML = '';
    cluster.themes.forEach(t => {
    ...
    if (cluster.sub_issues && cluster.sub_issues.length > 0) {
        ...
        subIssuesContainer.innerHTML = '<p style="color: var(--text-secondary); font-size: 13px;">No sub-issue decomposition available.</p>';
    }
