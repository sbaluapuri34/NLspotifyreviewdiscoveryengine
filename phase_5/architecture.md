...
* **Target Files**:
  - `backend/.env`: Config file for API keys (`YOUTUBE_API_KEY`, `GEMINI_API_KEY`, `APIFY_API_TOKEN`).
...
* **Key Tasks**:
  ...
  2. Implement a `.env` file containing `YOUTUBE_API_KEY`, `APIFY_API_TOKEN`, and `GEMINI_API_KEY`.
  3. Implement **Streaming Async Scrapers** configured to target the **India market** (where supported by the platform) with a volume of **10,000+ reviews** spanning the **last 6 months up to today**, strictly scoped to **Spotify-only** sources:
     ...
     - **Reddit Scraper**: Queries Spotify-specific subreddits: `r/truespotify`, `r/spotify`, `r/spotifyplaylist`, and `r/musicsuggestions`.
     - **Queue Injection**: All scrapers accept an `asyncio.Queue` and push reviews immediately as they are scraped.
     - **Reddit Scraper Token-Saving Optimizer**: Before triggering Apify, the pipeline queries the database's maximum Reddit post date ($Z$). If the user's requested `to_date` is $\le Z$, the engine counts existing reviews in that range. If we already have $X$ reviews and the user requested $Y$:
        * If $Y \le X$, it skips the Apify run entirely (`limit_reddit = 0`).
        * If $Y > X$, it reduces the scraping limit to the shortfall ($Y - X$), saving valuable Apify credits.
...
* **Key Tasks**:
  1. **Separate LLM & Key Configuration**: Initialize the engine using `GROQ_API_KEY_2` (the second key in the rotated list, or an explicit `api_key_2`) to isolate quota and rate limits from the main research engine.
...
* **Key Tasks**:
  ...
  6. **Research Questions Tab**: Implement cards for the 7 Research Questions that open modals containing the executive summary, key findings, and actionable product opportunities.
