...
class ResearchValidator:
    ...
    def validate_and_synthesize(self) -> Dict[str, Any]:
        ...
        evidence_packages = []
        packages_path = Path(self.db_path).parent / "scripts" / "compiled_evidence_packages.json"
        if packages_path.exists():
            try:
                with open(packages_path, "r", encoding="utf-8") as f:
                    evidence_packages = json.load(f)
            except Exception as e:
                logger.warning(f"ResearchValidator: Could not load compiled_evidence_packages.json: {e}")
...
Your goal is to perform high-level research validation and synthesis based on the compiled metrics, cluster intelligence, and research question answers.
...
2. TOP CLUSTER INTELLIGENCE & EVIDENCE:
