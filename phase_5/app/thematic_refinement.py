...
"""
    ...
    def validate_mappings(self, refined_themes: List[Dict[str, Any]], review_vectors: Dict[str, List[float]]) -> List[Dict[str, Any]]:
        ...
        vectors of ALL reviews in the database. Maps any review with similarity >= 0.60 to the theme.
        """
        validated_themes = []
        ...
        for theme in refined_themes:
            ...
            if theme_norm > 0:
                theme_vector = theme_vector / theme_norm  # Normalize
            
            verified_ids = []
            
            logger.info(f"ThematicRefinementEngine: Scanning all reviews for theme '{theme_name}'...")
            
            for rid, rev_vec in review_vectors.items():
                rev_vector = np.array(rev_vec)
                rev_norm = np.linalg.norm(rev_vector)
                if rev_norm <= 0:
                    continue
                rev_vector = rev_vector / rev_norm  # Normalize
                
                # Calculate cosine similarity
                similarity = float(np.dot(theme_vector, rev_vector))
                
                # Threshold of 0.60 for semantic match
                if similarity >= 0.60:
                    verified_ids.append(rid)
                    
            theme["verified_review_ids"] = verified_ids
            logger.info(f"  - Theme '{theme_name}': mapped {len(verified_ids)} reviews.")
